...
- Hobby: $5 minimum usage/month with $5 credits included (`pricing_model: "usage_token_minimum"`)
- Pro: $20 minimum usage/month with $20 credits included (`pricing_model: "usage_token_minimum"`)
...
4. If the response shape saves glue code or provider calls, inspect `commercial_next_step` and upgrade when usage-token minimum limits matter (`pricing_model: "usage_token_minimum"`; Hobby: $5 minimum usage/month with $5 credits included; Pro: $20 minimum usage/month with $20 credits included). Call `POST /api/v1/commercial/intent` from API-key context first if you want the paid-intent event attributed to the first-value request; current shell-only activation accounts still use browser signup/login for billing. The commercial handoff is not a provider booking flow.
